...
export default function AboutPage() {
  ...
  React.useEffect(() => {
    ...
    // Cinematic Crossfade Timer (starts transitioning after 3 seconds)
    ...
    <main className="relative bg-[#0A0A0A] min-h-screen text-white overflow-x-hidden">
      ...
      >
        <motion.div style={{ y }} className="absolute inset-0 z-0 bg-black">
          ...
          <Image
            src="/images/about/hero_interior_kr.jpg"
            alt="Premium Interior Atmosphere"
            fill
            className="object-cover opacity-60" 
            priority
          />
          {/* Exterior (Top Layer) - fades out */}
          <div 
            className="absolute inset-0 transition-opacity duration-[3000ms] ease-in-out z-10"
            style={{ opacity: showInterior ? 0 : 1 }}
          >
            <Image
              src="/images/about/hero_exterior_kr.jpg"
              alt="Premium Exterior Architecture"
              fill
              className="object-cover opacity-60"
              priority
            />
      ...
      <section className="min-h-screen w-full relative flex flex-col snap-start scroll-mt-[120px] overflow-hidden">
            ...
            <div className="flex-1 w-full order-2 lg:order-1 relative">
              ...
              >
                <Image
                  ...
                  fill
                  className="object-cover grayscale md:hover:grayscale-0 transition-all duration-1000"
                />
                {/* Highlight Box */}
      ...
      <section className="min-h-screen w-full relative flex flex-col snap-start scroll-mt-[120px] overflow-hidden bg-[#0A0A0A]">
        <div className="flex-1 flex flex-col justify-center px-4 md:px-8 max-w-[1400px] mx-auto w-full">
          <motion.div
            ...
            className="mb-8"
          >
            <span className="text-[10px] font-black tracking-[0.8em] text-white/30 uppercase block mb-2">Core Principles</span>
            <h2 className="text-3xl lg:text-5xl font-black text-white uppercase">라올을 움직이는 세 가지 원칙</h2>
          </motion.div>

          {/* 3x1 Image Grid for Visual Depth and Proper Section Height */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="relative aspect-[16/9] md:aspect-[4/3] max-h-[60px] lg:max-h-[200px] overflow-hidden rounded-sm opacity-50">
              <Image 
            ...
            <div className="relative aspect-[16/9] md:aspect-[4/3] max-h-[100px] lg:max-h-[200px] overflow-hidden rounded-sm opacity-50">
              <Image 
                src="https://images.unsplash.com/photo-1616137422495-1e9e46e2aa77?auto=format&fit=crop&q=80"
                alt="Bedroom detail"
                fill
                className="object-cover md:hover:scale-105 transition-transform duration-700"
              />
          ...
          {/* 4. Simplified About-Page Footer (to fit inside 3rd snap) */}
          <div className="mt-8 pt-8 border-t border-white/5 opacity-40">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 pb-8">
              <div className="text-xl font-bold tracking-tighter uppercase">LAOL <span className="font-light">| 실내건축</span></div>
              <div className="text-[10px] tracking-widest uppercase flex gap-8">
                <span>Instagram</span>
                <span>Blog</span>
              </div>
              <p className="text-[10px] text-white/40">© 2026 Laol Interior Architecture. All rights reserved.</p>
            </div>
          </div>
        </div>
      </section>
